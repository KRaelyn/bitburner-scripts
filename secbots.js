...
export async function main(ns) {
    ...
    while (true) {
      ...
      for (let s in servers) {
        ...
        let serverMinSec = ns.getServerMinSecurityLevel(server) + 2;
        let serverSecValue = ns.getServerSecurityLevel(server);
        let pLevel = ns.getHackingLevel();
        let sLevel = ns.getServerRequiredHackingLevel(server);
        let canHack = sLevel <= pLevel;
        while (serverSecValue > serverMinSec && canHack) {
          await ns.weaken(server);
          serverSecValue = ns.getServerSecurityLevel;
          await ns.sleep(1000);
        }
      }
      await ns.sleep(1000);
